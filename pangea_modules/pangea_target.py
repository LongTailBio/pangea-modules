...
class PangeaSampleTarget(PangeaTarget):
    """Target for luigi that corresponds to an
    AnalysisResultField for Pangea Server.
...
class PangeaGroupTarget(PangeaTarget):
    """Target for luigi that corresponds to an
    AnalysisResultField for Pangea Server.
    ...
    def __init__(
        self,
        server_address, group_name, module_name, field_name,
        is_s3=False, local=False, force_rebuild=False, ext=''
    ):
        self.group_name = group_name
        self.module_name = module_name
    ...
    def upload(self):
        ...
        if self.is_s3:
            field.upload()
            field = field.serializable()
        self.server.load_group_result_field(
            self.group_name,
            self.module_name,
            self.field_name,
            field
